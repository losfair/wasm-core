...
impl Continuation {
    ...
    fn write(&self, target: usize, opcodes: &mut [::opcode::Opcode]) {
        use ::opcode::Opcode;
...
impl Label {
    ...
    fn terminate(&self, opcodes: &mut [::opcode::Opcode]) {
...
pub fn translate_opcodes(ops: &[elements::Opcode]) -> Vec<::opcode::Opcode> {
    use self::elements::Opcode as PwOp;
    use ::opcode::Opcode as WcOp;

    let mut result: Vec<::opcode::Opcode> = Vec::new();
