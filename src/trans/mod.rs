...
use executor::RuntimeConfig;
use resolver::NullResolver;

pub fn translate_value_type(v: &elements::ValueType) -> ::module::ValType {
    match *v {
        elements::ValueType::I32 => ::module::ValType::I32,
        elements::ValueType::I64 => ::module::ValType::I64,
        elements::ValueType::F32 => ::module::ValType::F32,
        elements::ValueType::F64 => ::module::ValType::F64
    }
}

pub fn eval_init_expr(
    expr: &elements::InitExpr,
    globals: &mut Vec<::module::Global>
) -> ::value::Value {
    let mut code = optrans::translate_opcodes(expr.code());
    code.push(::opcode::Opcode::Return);

    let module = ::module::Module {
        types: vec! [
            ::module::Type::Func(Vec::new(), vec! [ ::module::ValType::I32 ])
        ],
        functions: vec! [
            ::module::Function {
                name: None,
...
pub fn translate_module_raw(
    ...
    let types: Vec<::module::Type> = if let Some(s) = module.type_section() {
        s.types().iter().map(|t| {
    ...
    let mut export_map: BTreeMap<String, ::module::Export> = BTreeMap::new();
        ...
        for entry in exports.entries() {
            ...
            match *internal {
                Internal::Function(id) => {
                    export_map.insert(
                        field.to_string(),
                        ::module::Export::Function(id as u32)
                    );
                },
                _ => {
    ...
    let mut functions: Vec<::module::Function> = Vec::new();
    let mut natives: Vec<::module::Native> = Vec::new();
    let mut globals: Vec<::module::Global> = Vec::new();
    let mut tables: Vec<::module::Table> = Vec::new();
            ...
            match *entry.external() {
                External::Function(typeidx) => {
                    ...
                    let patched = if config.emscripten.unwrap_or(false) && entry.module() == "env" {
                        let f: Option<::module::Function> = try_patch_emscripten_func_import(
                            entry.field(),
                            typeidx,
                            &types[typeidx],
                            &export_map
                    ...
                    if !patched {
                        ...
                        let ::module::Type::Func(ref ty, _) = types[typeidx];
    ...
    functions.extend((0..bodies.len()).map(|i| {
        ...
        let mut locals: Vec<::module::ValType> = Vec::new();
        ...
        opcodes.push(::opcode::Opcode::Return);
    ...
    let mut data_segs: Vec<::module::DataSegment> = Vec::new();
    if let Some(ds) = module.data_section() {
        for seg in ds.entries() {
            ...
            data_segs.push(::module::DataSegment {
                offset: offset,
...
fn try_patch_emscripten_global(field_name: &str) -> Option<::value::Value> {
    use ::value::Value;
...
fn try_patch_emscripten_func_import(
    ...
    typeidx: usize,
    ty: &::module::Type,
    export_map: &BTreeMap<String, ::module::Export>
) -> Option<::module::Function> {
    use ::module::{Function, FunctionBody};
    use ::opcode::Opcode;
...
fn gen_invoke_n_to_dyncall_n(
    ...
    use ::module::{Function, FunctionBody, Type, Export};
    use ::opcode::Opcode;
